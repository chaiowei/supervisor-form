...
### Notion 日誌新增照片直接顯示（不只是連結）
- 新增 `Build Photo Blocks` → `Notion: Append Photos` 兩個節點：日誌頁面建立後，把 `photoUrls` 逐一轉成 Notion 圖片區塊（image block）附加到頁面內容下方，照片會直接顯示在頁面裡，不再只是「Photo URLs」欄位裡的一串純文字連結。
- 沒有照片時這兩個節點不會執行（不會產生空白圖片區塊）。
- `Photo URLs` 欄位保留，仍會寫入所有照片連結文字，方便搜尋/複製；圖片顯示是額外附加，不影響原本欄位。
